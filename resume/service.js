...
//JobDeer 工具
	...
	return {
		restrict: "EA",
		replace: true,
		template: '<ul class="pagination" ng-if="show"><li ng-click="go({page:1})"><a href="javascript:void(0);" title="首页" target="_blank" rel="external">&laquo;</a></li><li ng-repeat="page in pages" ng-class="{active:page==currentpage}" ng-click="go({page:page})"><a href="javascript:void(0);" target="_blank" rel="external"></a></li><li ng-click="go({page:totalpage})"><a href="javascript:void(0);" title="尾页" target="_blank" rel="external">&raquo;</a></li></ul>',
		scope: {
			//总页数
			totalpage: "=",
			//当前页数
		...
		controller: function ($scope, $element, $attrs, $transclude) {
			var run = function () {
				...
				var pages = [];
				for (i = start_page; i <= end_page;="" i++)="" {="" pages.push(i);="" }="" $scope.pages="pages;" $scope.totalpage="totalpage;" $scope.currentpage="currentpage;" };="" $scope.$watch('totalpage+currentpage',="" function="" ()="" 计算更改pages变量="" run();="" });="" })="" .directive('loadstatus',="" ($timeout)="" return="" restrict:="" "ea",="" replace:="" false,="" scope:="" loadtext:="" "@loadstatus"="" },="" link:="" (scope,="" element,="" attrs,="" ngmodel)="" 监听元素点击事件="" $(element).on('click',="" $timeout(function="" 立即disabled会导致表单不能提交，所以timeout一下="" $(element).prop('disabled',="" true);="" 0)="" if="" (scope.loadtext)="" var="" oldvalue="$(element).val();" is_val="true;" (!oldvalue)="" val="" 获得不了用html="" 加载中的文字="" (is_val)="" $(element).val(scope.loadtext);="" else="" $(element).html(scope.loadtext);="" false);="" $(element).val(oldvalue);="" $(element).html(oldvalue);="" 2000);="" .factory('jdmodal',="" ($rootscope,="" $http,="" $templatecache,="" $compile,="" $controller)="" ($('#jobdeer-modal').size()="=" $('<div="" class="modal fade" id="jobdeer-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"></div></div>').appendTo('body');
	}
				...
				if (scope.ghostDown) {
					...
					if (value) {
						...
						if ('1' === scope.canWeixinScan) {
							weixin_scan = ' 或者 <a href="javascript:void(0);" ng-click="weixin_request()" target="_blank" rel="external">微信扫码求简历</a>    ';
						}
						$(element).html('<div class="resume_content">' + html + '</div>');
						$(element).find('p:contains("[hidden]")').html('<div class="password"> <h3>此段内容已被阅读密码保护</h3> 您可以  <a href="javascript:void(0);" ng-click="input_password()" target="_blank" rel="external">输入密码</a>' + weixin_scan + '</div>');
						$(element).find('p:contains("[error]")').html('<div class="password error"> <h3>密码错误</h3> 您可以  <a href="javascript:void(0);" ng-click="input_password()" target="_blank" rel="external">输入密码</a> ' + weixin_scan + '   </div>');
